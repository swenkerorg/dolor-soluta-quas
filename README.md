...
`npm install @swenkerorg/dolor-soluta-quas`

```js
import jsTokens from "@swenkerorg/dolor-soluta-quas";
// or:
const jsTokens = require("@swenkerorg/dolor-soluta-quas");
```
...
Differentiating between regex and division in JavaScript is really tricky. @swenkerorg/dolor-soluta-quas looks at the previous token to tell them apart. As long as the previous tokens are valid, it should do the right thing. For invalid code, @swenkerorg/dolor-soluta-quas might be confused and start matching division as regex or vice versa.
...
- Numeric literals: @swenkerorg/dolor-soluta-quas supports legacy octal and octal like numeric literals, regardless of Strict Mode.
- String literals: @swenkerorg/dolor-soluta-quas supports legacy octal escapes, since it allows any invalid escapes.
- HTML-like comments: **Not supported.** @swenkerorg/dolor-soluta-quas prefers treating `5<!--x` as `5 < !(--x)` rather than as `5 //x`.
- Regular expression patterns: @swenkerorg/dolor-soluta-quas doesn’t care what’s between the starting `/` and ending `/`, so this is supported.

### TypeScript

Supporting TypeScript is not an explicit goal, but @swenkerorg/dolor-soluta-quas and Babel both tokenize this [TypeScript fixture] and this [TSX fixture] the same way, with one edge case:
...
Both lines above should end with a couple of `>` tokens, but @swenkerorg/dolor-soluta-quas instead matches the `>>` and `>>>` operators.
...
@swenkerorg/dolor-soluta-quas should work in any JavaScript runtime that supports [Unicode property escapes].
...
But @swenkerorg/dolor-soluta-quas thinks they mean:
...
In other words, @swenkerorg/dolor-soluta-quas:
...
This happens because @swenkerorg/dolor-soluta-quas looks at the previous token when deciding between regex and division or JSX and comparison. In these cases, the previous token is `}`, which either means “end of block” (→ regex/JSX) or “end of object literal” (→ division/comparison). How does @swenkerorg/dolor-soluta-quas determine if the `}` belongs to a block or an object literal? By looking at the token before the matching `{`.
...
@swenkerorg/dolor-soluta-quas advertises that it “never fails”. Tell you what, it _can_ fail on extreme inputs. The regex engine of the runtime can eventually give up. @swenkerorg/dolor-soluta-quas has worked around it to some extent by changing its regexes to be easier on the regex engine. To solve completely, @swenkerorg/dolor-soluta-quas would have to stop using regex, but then it wouldn’t be _tiny_ anymore which is the whole point. Luckily, only extreme inputs can fail, hopefully ones you’ll never encounter.
...
```js
Array.from(require("@swenkerorg/dolor-soluta-quas")(`"${"\\n".repeat(1e7)}"`));
```

(Yes, that is the _regex engine_ of the runtime giving up. @swenkerorg/dolor-soluta-quas has no recursive functions.)

However, if you repeat `a` instead of `\n` 10 million times (`"aaaaaa…"`), it works:

```js
Array.from(require("@swenkerorg/dolor-soluta-quas")(`"${"a".repeat(1e7)}"`));
```
...
This means that in Safari, @swenkerorg/dolor-soluta-quas might not fail but instead give you unexpected tokens.

## Performance

With [@babel/parser] for comparison. Node.js 21.6.1 on a MacBook Pro M1 (Sonoma).

| Lines of code |     Size | @swenkerorg/dolor-soluta-quas@8.0.3 | @babel/parser@7.23.9 |
| ------------: | -------: | --------------: | -------------------: |
